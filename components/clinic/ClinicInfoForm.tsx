...
import '@/app/styles/info-klinik.css';
...
export interface ClinicInfoFormProps {
  ...
  subtitle?: string;
  canEdit: boolean;
  fetchClinic: () => Promise<ClinicResponse>;
  updateClinic: (payload: UpdateClinicPayload) => Promise<unknown>;
  uploadLogo: (file: File) => Promise<{ logoUrl: string }>;
  /** Rendered in the right column above "Jam Operasional" — e.g. a subscription card. */
  rightColumnExtra?: ReactNode;
}

/**
 * The clinic identity/address/hours form itself, shared between the regular
 * owner's Info Klinik page and the multi-klinik owner's per-clinic Info
 * Klinik page — the two differ only in which clinic the fetch/update/upload
 * functions are scoped to (own clinic vs. a chosen linked clinic by id).
 */
export default function ClinicInfoForm({
  title = 'Info Klinik',
  subtitle = 'Informasi dan pengaturan klinik',
  canEdit,
  fetchClinic,
  updateClinic,
  uploadLogo,
  ...
  const [days, setDays] = useState<ClinicDayHours[]>(operationalHoursToDays());
  ...
  function updateDay(id: string, patch: Partial<ClinicDayHours>) {
          ...
          <div>
            ...
            <div className="card">
              ...
              <div className="hours-list">
                {days.map((d) => {
                  const canEditDay = isEditing && d.active;
                  return (
                    <div
                      key={d.id}
